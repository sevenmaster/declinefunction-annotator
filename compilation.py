...
def _get_binary_name(source_file: str, compiler: str, optimization_level) ->\
        str:
    return source_file[:-4] + '_' + compiler + '_' + optimization_level[1:]
...
def compile_with_symbols(source_file: str, binary_dir, compiler: str,
    ...
    binary_file = os.path.join(binary_dir,
    ...
    if os.path.exists(binary_file) and\
        ...
        return binary_file
    command = [compiler, optimization_level, '-g3', source_file, '-o',
               binary_file]
...
def compile_all(source_file: str, binary_dir: str):
    results = []
    for compiler in compilers:
        for optimization_level in optimization_levels:
            binary_path = compile_with_symbols(source_file,
                                               binary_dir,
                                               compiler,
                                               optimization_level)
            results.append(binary_path)
    return results
